...
function TypeVector(options) {
  ...
  if (TypeVector.logger.isDebugEnabled()) {
    TypeVector.logger.debug(
      "Vector init: module = %s , type = %s, list = %s",
      opts.module,
      opts.type,
      this.list
    );
...
function manageType(type) {
  ...
  if (type.charAt(0) === type.charAt(0).toUpperCase()) {
    ...
    var BareType = isBare
      ? tl.requireTypeByName(this.module + "." + type)
      : null;
    this["read" + type] = function() {
      var Type = BareType
        ? BareType
        : tl.requireTypeFromBuffer(this._buffer.slice(this.getReadOffset()));
      var obj = new Type({
        buffer: this._buffer,
        offset: this.getReadOffset()
      });
