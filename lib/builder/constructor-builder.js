...
var { capitalize } = require("../utility");
...
function retrieveTypeName(tlSchema) {
  ...
  return typeName.substring(0, idx) + capitalize(typeName.substring(idx));
}
  ...
  if (this.tlSchema.params) {
    for (var i = 0; i < this.tlSchema.params.length; i++) {
      ...
      if (isCapitalized(typeName)) {
        body += buildWriteObjectProperty.call(
          this,
          param.name,
          typeName,
          isBare
      ...
      // Manage primitive type
        ...
        body +=
          "\t\tthis." +
          buildWriteProperty.call(this, param.name, typeName) +
          "();\n";
      }
    }
  }
...
function buildWriteObjectProperty(propertyName, typeName, isBare) {
  ...
  if ("Object" === typeName) {
    ...
    body +=
      "\t\tthis." + buildWriteProperty.call(this, "bytes", "Int") + "();\n";
  }
...
function buildWriteProperty(propertyName, typeName) {
  var functionName = "__write" + capitalize(propertyName);
  var body =
    "\tthis." + functionName + " = function " + functionName + "() {\n";
  body +=
    "\t\tif(this.constructor.logger.isDebugEnabled()) {\n" +
    "\t\t\tthis.constructor.logger.debug('write \\'%s\\' = %s', '" +
    propertyName +
    "', this." +
    propertyName +
  ...
  if (this.tlSchema.params) {
    for (var i = 0; i < this.tlSchema.params.length; i++) {
      ...
      if (!type[3]) {
        ...
        // Manage Object type
        if (isCapitalized(typeName)) {
          body += buildReadObjectProperty.call(
            this,
            param.name,
            typeName,
            isBare
        ...
        // Manage primitive type
          ...
          body +=
            "\t\tthis." +
            buildReadProperty.call(this, param.name, typeName) +
            "();\n";
        }
      }
      // Manage generic type
      else {
        typeName = capitalize(type[4]) + capitalize(type[2]);
        body +=
          "\t\tvar " +
          typeName +
          " = this.constructor.requireTypeByName('" +
          typeName +
          "');\n" +
          buildDeserializeObjectProperty.call(
            this,
            param.name,
            typeName,
            type[3],
  ...
  if ("X" === typeName) {
    ...
    body += !isBare
      ? "\t\tvar bytes = this._buffer.slice(this.getReadOffset(), this.getReadOffset() + 4);\n" +
        "\t\tif(this.constructor.logger.isDebugEnabled()) {\n" +
        "\t\t\tthis.constructor.logger.debug('Retrieve type %s reading the bytes %s', " +
        typeName +
        ",  bytes.toString('hex'));\n" +
        "\t\t}\n" +
        "\t\tvar " +
        typeName +
        " = this.constructor.requireTypeFromBuffer(bytes);\n"
      : "\t\tif(this.constructor.logger.isDebugEnabled()) {\n" +
        "\t\t\tthis.constructor.logger.debug('Retrieve BARE type by name %s', '" +
        fullTypeName +
        "');\n" +
        "\t\t}\n" +
        "\t\tvar " +
        ...
        fullTypeName +
        "');\n";
    body += buildDeserializeObjectProperty.call(
      this,
      propertyName,
      typeName,
      null,
...
function buildDeserializeObjectProperty(
  propertyName,
  typeName,
  typeParam,
  isBare
  ...
  return (
    ...
    "({" +
    (typeParam
      ? "module: '" + this.module + "', type: '" + typeParam + "', "
      : "") +
    ...
    "\t\t\t}\n" +
    "\t\t} else {\n" +
    "\t\t\tthrow new TypeError('Unable to retrieve the Type constructor for the type " +
    typeName +
    " and buffer:' + this._buffer.toString('hex'));\n" +
    "\t\t}\n"
  );
}

function buildReadProperty(propertyName, typeName) {
  var functionName = "__read" + capitalize(propertyName);
  var body =
    "\tthis." + functionName + " = function " + functionName + "() {\n";
  body += "\t\tthis." + propertyName + " = this.read" + typeName + "();\n";
  body +=
    "\t\tif(this.constructor.logger.isDebugEnabled()) {\n" +
    "\t\t\tthis.constructor.logger.debug('read \\'%s\\' = %s, offset = %s', '" +
    propertyName +
    "', this." +
    propertyName +
    ("Bytes" === typeName ? ".toString('hex')" : "") +
