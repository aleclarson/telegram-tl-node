...
TypeObject.prototype.deserialize = function(options) {
  ...
  if (!this._noId && !isBare) {
    ...
    if (this._typeId !== id) {
      var msg =
        "Unable to de-serialize, (read id) " +
        id +
        " != (this._typeId) " +
        this._typeId;
      logger.error(msg);
...
TypeObject.prototype._writeIntN = function(bigInteger, byteLength) {
  return typeof bigInteger === "string" || typeof bigInteger === "number"
    ? this._writeBigInt(bigInteger, byteLength)
...
TypeObject.prototype.readBytes = function(useWordLength) {
  ...
  if (!isShort) {
    bLength =
      this._buffer.readUInt8(this._readOffset++) +
